...
CREATE TABLE `student_ensantos`.`Users` (
...
CREATE TABLE `student_ensantos`.`RoommateInfo` (
...
CREATE TABLE `student_ensantos`.`Student` (
...
CREATE TABLE `student_ensantos`.`LeaseHolder` (
  `leaseholder_id` int(11) NOT NULL AUTO_INCREMENT,
  `user_id` int NOT NULL,
  PRIMARY KEY (`leaseholder_id`),
  FOREIGN KEY (`user_id`) REFERENCES `Users`(`user_id`)); /* FK */

CREATE TABLE `student_ensantos`.`Picture` (
  `picture_id` int(11) NOT NULL AUTO_INCREMENT,
  `image_file` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`picture_id`));

CREATE TABLE `student_ensantos`.`Address` (
...
CREATE TABLE `student_ensantos`.`Listings` (
